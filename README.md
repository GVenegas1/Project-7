...
[Because correctly manipulating linked lists can be especially tricky for beginning programmers and because there are a number of special cases to check for, more test results will be immediately visible to you than usual, however, there are still some that will only be visible after the due date.]
...
All the methods should have the arguments in the same order as you saw in the Exploration. You may use default arguments and/or helper functions.

Hint: Remember that you can reference the iterative versions of `add` and `remove` in the Exploration and the iterative versions of `contains`, `insert`, and `reverse` in the Exercise solutions. Your recursive implementations should have the same behavior as those iterative implementations, so comparing with them may be useful.
